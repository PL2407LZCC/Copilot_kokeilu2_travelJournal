...
router.get("/", authMiddleware, (req, res) => {
  ...
  const query = `
    ...
    LEFT JOIN country_status cs ON je.country_code = cs.country_code AND cs.user_id = ?
    WHERE je.user_id = ?
...
// Get journal entries for a specific country (for authenticated user)
router.get("/country/:countryCode", authMiddleware, (req, res) => {
  ...
  const query = `
    ...
    FROM journal_entries je
    LEFT JOIN country_status cs ON je.country_code = cs.country_code AND cs.user_id = ?
    WHERE je.country_code = ? AND je.user_id = ?
    ORDER BY je.created_at DESC
  `;

  db.all(query, [userId, countryCode, userId], (err, rows) => {
    if (err) {
...
router.post("/", authMiddleware, (req, res) => {
  ...
  db.serialize(() => {
    // First, update or insert country status for this user
    const upsertCountryStatus = `
      INSERT OR REPLACE INTO country_status (country_code, country_name, visit_status, user_id, updated_at)
      VALUES (?, ?, ?, ?, CURRENT_TIMESTAMP)
    `;

    db.run(
      upsertCountryStatus,
      [countryCode, countryName, visitStatus || "not-visited", userId],
      function (err) {
        if (err) {
...
// Update a journal entry (requires authentication and ownership)
router.put("/:id", authMiddleware, (req, res) => {
  ...
  db.run(updateQuery, [entry, visitStatus, id, userId], function (err) {
    if (err) {
    ...
    db.get(selectQuery, [userId, id, userId], (err, row) => {
      if (err) {
...
// Delete a journal entry (requires authentication and ownership)
